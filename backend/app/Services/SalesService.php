...
class SalesService
{
    ...
    public function confirmDelivery(int $saleId): array
    {
        ...
        if ($quantidade > $estoqueAtual) {
            throw new \RuntimeException('Estoque insuficiente para confirmar a entrega', 409);
        }

        $novoEstoque = $estoqueAtual - $quantidade;
        $this->productRepo->updateStock($produtoId, $novoEstoque);
        $this->salesRepo->updateStatus($saleId, 'ENTREGUE');

        Logger::get()->info('Venda entregue', ['sale_id' => $saleId, 'novo_estoque' => $novoEstoque]);

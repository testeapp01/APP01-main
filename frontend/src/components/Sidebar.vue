  ...
  <aside class="w-64 bg-slate-900 text-white min-h-screen p-4 hidden md:block">
    ...
    <nav class="space-y-2">
      ...
      >
        {{ item.label }}
        <span
          v-if="loadingItem === item.label"
          class="absolute right-2 top-2 h-3 w-3 border-2 border-white border-t-transparent rounded-full animate-spin"
        />
      </a>
      <button
        class="w-full text-left block px-3 py-2 rounded hover:bg-slate-800 mt-4 text-sm"
        @click="logout"
      >
        Sair
      </button>
